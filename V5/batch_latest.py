...
config_batch = json.load(open('batch_config.json', 'r'))
...
class BatchGenerator:
    ...
    def get_token(self):
        try:
            url = config_batch['url_auth']
            data = config_batch['user_data']
    ...
    def data_getting(self, filename):
        try:
            ...
            api_url_web = config_batch["api_url_web"]
            for f1 in filename:
                data_config = json.load(open(f1, 'r'))
                for art in data_config['articles']:
                    if art['status'] is False and art['newspaper'] and art['headline'] and art['article_date'] \
                            and art['article_time'] != '' and art['story']:
                            ...
                            if res_con['status'] == 201:
                                for art in data_config['articles']:
                                    if art['status'] is False and res_con['jobIdentifier'] == art['url']:
                                        art['status'] = True
                                        batch_log.info('%s - %s', "STATUS IS CHANGING OF NEWS", art['newspaper'])
    ...
    def main(self):
        try:
            ...
            batch_log.info('%s ', "BATCH START")
            kls = []
            ols = []
            for web in json_list:

                if web.split('/')[-6] in config_batch['channels']:
                    kls.append(web)
                else:
                    ols.append(web)
            check = kls[0].split('/')[-6]
            ...
            for kl in kls:
                if check == kl.split('/')[-6]:
                    ...
                    self.getting_folders(tls)
                    tls = []
                    tls.append(kl)
                    check = kl.split('/')[-6]
                    t.sleep(60)

            self.getting_folders(ols)
            batch_log.info('%s ', "BATCH END")
        except Exception as exp:
            batch_log.info('%s -%s', "EXCEPTION IN MAIN CALL", exp)
