...
class Scrappers:
    ...
    def scrapped_multiple(self):
        try:
            for i in range(len(self.website_url)):
                ...
                self.get_scrapped_data(self.website_url[i], self.category[i], self.ele_div[i])
        except Exception as exp:
    ...
    def get_scrapped_data(self, url_web, category_web, ele_soup):
        try:
            ...
            self.get_category_links(dictionary, category_web, ele_soup)
    ...
    def get_category_links(self, dictionary, category_website, ele_soup):
        try:
            for k, v in dictionary.items():
                if k == category_website:
                    ...
                    if self.news == "Daily Times Pakistan" or self.news == "Business Recorder":
                        ...
                        self.get_article_data(dictionary, elements, ref, category_website)
    ...
    def get_article_data(self, dictionary, elements, ref, category_website):
        global href
        try:
            # print(ref)
    ...
    def get_data(self, href, response, element, category_website):
        try:
            soup = bs(response.content, 'html.parser')
    ...
    def get_body(self, article_soup):
        try:
            if self.body_type == "p":
                ...
                if self.news == "Business Recorder":
                    to_replace = article_soup.find('div', self.body_div).find('a').text.strip()
                    body_full = article_soup.find('div', self.body_div).text.strip()
                    body = body_full.replace(to_replace, "")
                else:
    ...
    def get_image(self, article_soup, image_file_path, title, category_website):
        try:
            if self.news == "The Nation" or self.news == "The Express Tribune" or self.news == "Hum News" or self.news == "Daily Express News" or self.news == "Daily Times Pakistan" or self.news == "The Nawaiwaqt" or self.news == 'Daily Pakistan' or self.news == 'Daily Dunya News' or self.news == 'Geo News' or self.news == '92 News' or self.news == 'Business Recorder' or self.news == 'Jasarat' or self.news == 'Daily Ummat' or self.news == 'Financial Daily' or self.news == "Financial Daily General" or self.news == 'Jurat' or self.news == "PPI" or self.news == "Pakistan Observer" or self.news == "NNI" or self.news == "Pakistan Today" or self.news == "Qaumi Akbar" or self.news == "APP URDU":
                ...
                if category_website == "Energy":
                    image_div = article_soup.find("div", "styles__image-container__skIG1 styles__fill__3xCr1 styles__center_center__1AaPV styles__apply-ratio__1_FYQ")
                else:
                    image_div = article_soup.find("div", self.image_div)
            else:
            ...
            if image_div:
                ...
                elif self.news == "Daily Times Pakistan" or self.news== "Pakistan Observer":
